...
def far_field(dataset, a):
    ...
    F = np.array([
        -coef1*np.cos(beta)*np.imag(H_beta) - coef2*trapezoid(np.real(H)*np.imag(np.conjugate(H))*np.cos(theta), theta),
...
if __name__ == "__main__":
    ...
    water_depth = 200
    radius = 1
    theta_range = np.linspace(-np.pi/2, np.pi/2, 19)
